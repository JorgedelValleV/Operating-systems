...
{
	char* name=(char*)(filename+1); //copiamos filename en una variable no constante obligado por findFileByName,  +1 => quitar 1er caracter '/' (mueve el puntero)
	
	int idxDir;// índice que ocupa un nodo en el array de nodos
	 
	if ((idxDir = findFileByName(&myFileSystem, name) )== -1) { // Buscamos el archivo en nuestro directorio
		fprintf(stderr, "File wasn't found on the directory entry\n");  
		return -1;
	}
	
	// El archivo esta en el directorio-->liberamos la entrada del directorio (pasa a estar libre)

	myFileSystem.directory.files[idxDir].freeFile = true; // El nodo asociado pasa a estar libre
	myFileSystem.directory.numFiles -= 1; // Un archivo menos en el directorio
	updateDirectory(&myFileSystem);

    // Copia del inodo asociado al archivo
	int idxNode = myFileSystem.directory.files[idxDir].nodeIdx; // indice inodo asoc -> acceder en el array
	NodeStruct* inode=myFileSystem.nodes[idxNode];

    // Liberamos el nodo y borramos los datos

    //Borramos un nodo -> pasa a estar libre
	myFileSystem.numFreeNodes += 1; 

    /******************* 
    // Liberamos bloques de datoS
	...
	inode->fileSize=0;
    ***********************/
    // Lo anterior equivale al resize
	inode->freeNode=true;

    resizeNode(idxNode, 0);
	updateNode(&myFileSystem,idxNode,inode);
	
    //Borramos el nodo del array
	free(inode);
	myFileSystem.nodes[idxNode]=NULL;

	fprintf(stderr, "\n Eres un genio!!! %s borrado \n", filename);
 ...
 *
 * @param path file path
 * @param buf buffer where we save the data read
 * @param size quantity of bytes to read
 * @param offset offset over the reading
 * @param fi FUSE structure linked to the opened file
 * @return size on success and <0 on error
 **/
int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) 
{
	...
	NodeStruct *node = myFileSystem.nodes[fi->fh]; // Linea 271 para ver que es file handle

    // Comprobar que el offset no se sale fuera del fichero?
    if (offset > node->fileSize){
        return 0;
    }
    
    if(node->fileSize<(bytes2Read + offset)){ // size + offset // Por si leemos 100B de un fichero de 10B
        //bytes2Read=node->fileSize;
        bytes2Read=node->fileSize-offset;
    }
    int toRead = bytes2Read;
	...
	while(bytes2Read) {
		...
		// Guardamos en buf lo que hemos leido y almacenado en buffer
        // hasta que completemos el bloque o hayamos terminado la lectura
		for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalRead < toRead); i++) {//size por toRead y la segunda condicion equivale a (totalRead - offset < bytesToRead)
